import { useState, useRef, useEffect } from "react";

// NSW Health Pathology logo SVG path data (shared between header and message avatars)
const NswLogo = ({ size = 24 }: { size?: number }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 259 247"
    xmlns="http://www.w3.org/2000/svg"
    fill="currentColor"
  >
    <path d="M146.699 209.236C145.217 206.841 142.982 204.665 140.059 202.771C137.14 200.877 133.162 199.431 128.249 198.474L114.571 195.66C110.439 194.769 107.59 193.617 106.103 192.241C104.654 190.898 103.919 189.112 103.919 186.932C103.919 185.544 104.235 184.28 104.862 183.173C105.485 182.062 106.39 181.08 107.549 180.251C108.715 179.423 110.165 178.768 111.855 178.308C113.566 177.839 115.488 177.603 117.569 177.603C120.309 177.603 122.78 177.964 124.915 178.681C127.024 179.394 128.772 180.562 130.109 182.162C131.447 183.762 132.269 185.937 132.551 188.627L132.589 189H149.56L149.552 188.573C149.477 183.998 148.181 179.8 145.702 176.099C143.219 172.394 139.561 169.418 134.823 167.247C130.097 165.083 124.2 163.985 117.295 163.985C111.494 163.985 106.178 164.996 101.49 166.994C96.7893 168.996 93.0146 171.884 90.278 175.581C87.529 179.298 86.1587 183.745 86.2002 188.805C86.3123 194.98 88.1893 199.991 91.7771 203.708C95.3525 207.405 100.253 209.866 106.344 211.014L120.139 213.824C122.668 214.326 124.964 214.984 126.962 215.784C128.922 216.568 130.487 217.587 131.613 218.814C132.713 220.011 133.27 221.586 133.27 223.484C133.27 225.598 132.576 227.384 131.21 228.785C129.81 230.219 127.909 231.3 125.567 232.009C123.179 232.73 120.509 233.095 117.631 233.095C114.824 233.095 112.237 232.66 109.945 231.798C107.665 230.948 105.751 229.705 104.256 228.105C102.765 226.514 101.743 224.52 101.216 222.187L101.141 221.86H83.8457L83.8914 222.32C84.2983 226.365 85.5566 229.949 87.6287 232.979C89.6967 235.988 92.346 238.524 95.502 240.53C98.6496 242.523 102.179 244.044 105.996 245.043C109.803 246.037 113.74 246.543 117.689 246.543C124.155 246.543 129.889 245.524 134.735 243.509C139.602 241.487 143.452 238.702 146.184 235.233C148.937 231.74 150.332 227.757 150.332 223.385C150.332 221.321 150.1 219.025 149.635 216.563C149.157 214.081 148.169 211.615 146.699 209.236ZM25.9368 191.763L62.8532 245.075H77.8689V165.351H61.0717V216.633L25.7541 165.53L25.6295 165.351H9.18945V245.075H25.9368V191.763ZM222.574 216.34L236.962 165.353H253.419L229.832 245.077H216.465L201.911 194.608L187.252 245.077H174.105L150.572 165.353H167.083L181.467 216.332L195.856 165.353H208.023L222.574 216.34Z" />
    <path d="M122.291 145.572C102.541 143.148 83.3565 149.476 49.6002 140.309C46.1536 139.372 44.8829 143.003 46.7474 146.045C55.8166 160.848 101.628 149.248 122.382 147.11C123.288 147.015 123.196 145.684 122.291 145.572Z" />
    <path d="M208.735 140.309C174.979 149.476 155.794 143.148 136.044 145.572C135.139 145.684 135.044 147.019 135.953 147.11C156.712 149.248 202.519 160.848 211.588 146.045C213.452 143.003 212.182 139.372 208.735 140.309Z" />
    <path d="M59.8819 119.132C53.0177 109.074 47.4076 97.8218 43.064 85.4429C29.9626 89.268 16.554 94.7882 2.90445 102.012C1.13961 102.944 0.0557817 104.693 0.00179824 106.682C-0.0480326 108.672 0.940279 110.47 2.65529 111.494C29.0864 127.296 54.9279 136.335 79.5858 138.427C72.2441 133.952 65.5128 127.383 59.8819 119.132Z" />
    <path d="M23.0586 85.5128C29.1047 83.0967 35.0969 81.0245 41.0351 79.2798C38.909 72.3837 37.1649 65.1603 35.7862 57.6218C28.9594 56.7723 21.8959 56.2211 14.5873 55.9807C14.525 55.9807 14.4586 55.9766 14.3963 55.9766C12.482 55.9766 10.7545 56.938 9.74958 58.5792C8.71144 60.27 8.66576 62.3214 9.62916 64.062C13.8731 71.7206 18.3537 78.8695 23.0586 85.5128Z" />
    <path d="M88.0053 135.572C92.1039 137.379 96.298 138.39 100.363 138.601C90.4097 131.481 82.5904 119.409 78.0848 104.021C72.2795 84.2118 70.3403 62.3549 72.2546 38.774C64.4228 33.8382 55.935 29.2173 46.7952 24.9239C45.0303 24.095 43.0122 24.2566 41.3927 25.3549C39.7773 26.449 38.8845 28.26 39.0049 30.1954C41.1311 64.4105 49.9553 93.1096 65.2285 115.493C71.6317 124.884 79.5092 131.825 88.0053 135.572Z" />
    <path d="M107.488 23.5772C102.803 17.8995 97.7207 12.2841 92.2352 6.73487C91.1721 5.65736 89.8018 5.08545 88.3857 5.08545C87.8791 5.08545 87.3684 5.16005 86.8617 5.30924C84.9391 5.87701 83.548 7.41453 83.1286 9.4245C82.0572 14.5883 80.0183 25.1147 79.1172 35.5251C86.1309 40.1377 92.6089 45.0155 98.5429 50.1461C100.843 41.3893 103.833 32.5164 107.488 23.5772Z" />
    <path d="M255.42 102.007C241.77 94.7836 228.362 89.2634 215.26 85.4382C210.917 97.8172 205.307 109.069 198.442 119.127C192.812 127.382 186.08 133.947 178.734 138.423C203.392 136.33 229.234 127.291 255.665 111.489C257.38 110.465 258.372 108.667 258.318 106.678C258.273 104.688 257.185 102.944 255.42 102.007Z" />
    <path d="M235.273 85.5133C239.978 78.87 244.459 71.717 248.703 64.0584C249.666 62.3178 249.625 60.2705 248.582 58.5755C247.577 56.9385 245.85 55.9729 243.936 55.9729C243.873 55.9729 243.807 55.9729 243.745 55.977C236.436 56.2174 229.368 56.7727 222.546 57.6182C221.171 65.1566 219.423 72.3801 217.297 79.2761C223.235 81.025 229.231 83.1013 235.273 85.5133Z" />
    <path d="M180.247 104.025C175.742 119.409 167.918 131.485 157.969 138.605C162.034 138.394 166.232 137.378 170.327 135.571C178.823 131.825 186.7 124.883 193.104 115.496C208.377 93.1132 217.201 64.4142 219.327 30.199C219.448 28.2637 218.555 26.4526 216.939 25.3585C215.32 24.2603 213.302 24.0987 211.537 24.9275C202.397 29.2169 193.909 33.8377 186.077 38.7777C187.992 62.3586 186.048 84.2113 180.247 104.025Z" />
    <path d="M179.214 35.5207C178.313 25.1145 176.274 14.588 175.203 9.42011C174.787 7.41013 173.392 5.87261 171.47 5.30484C170.963 5.15565 170.452 5.08105 169.946 5.08105C168.53 5.08105 167.159 5.65296 166.096 6.73047C160.611 12.2796 155.528 17.8951 150.844 23.5728C154.498 32.512 157.488 41.3849 159.793 50.1376C165.727 45.0111 172.2 40.1333 179.214 35.5207Z" />
    <path d="M129.144 87.2286C135.784 74.1368 146.39 62.4003 154.354 54.9862C149.982 37.2073 143.25 20.4354 133.957 2.88856C132.998 1.07751 131.195 0 129.148 0C127.101 0 125.303 1.08165 124.34 2.88856C114.951 20.4934 108.103 38.0112 103.963 54.982C112.276 62.6696 123.243 75.2061 129.144 87.2286Z" />
    <path d="M153.541 133.686C160.426 128.853 167.311 120.432 171.68 108.716C180.138 86.0347 181.014 60.7712 179.802 42.8928C164.471 53.1085 140.137 74.4432 131.827 97.2532C127.916 107.987 126.093 122.608 129.107 130.764C130.366 134.175 132.508 136.761 135.452 137.991C139.912 139.856 146.764 138.447 153.541 133.686Z" />
    <path d="M125.713 95.261C123.686 90.5366 121.452 86.0815 117.985 81.0752C108.176 66.9142 94.9588 54.1416 78.5104 42.8899C78.3402 45.5422 75.94 74.1874 84.3614 102.194C90.7688 123.496 101.715 132.174 107.715 135.527C113.753 138.896 119.276 139.692 125.754 138.084C119.189 129.149 120.007 110.814 125.713 95.261Z" />
  </svg>
);

// ── Bot avatar ────────────────────────────────────────────────────────────────
function BotAvatar({ size = 32 }: { size?: number }) {
  return (
    <div
      style={{
        width: size,
        height: size,
        backgroundColor: "#ffffff",
        borderRadius: "50%",
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        flexShrink: 0,
        border: "1px solid #E4E7EB",
      }}
    >
      <NswLogo size={size === 32 ? 18 : 24} />
    </div>
  );
}

// ── Types ─────────────────────────────────────────────────────────────────────
interface Message {
  id: string;
  text: string;
  timestamp: string;
  isBot: boolean;
}

function formatTime(date: Date) {
  return date.toLocaleTimeString("en-AU", {
    hour: "2-digit",
    minute: "2-digit",
    second: "2-digit",
    hour12: false,
  });
}

// ── ChatbotBubble component ───────────────────────────────────────────────────
export function ChatbotBubble({
  isOpen,
  onClose,
}: {
  isOpen: boolean;
  onClose: () => void;
}) {
  const [minimised, setMinimised] = useState(false);
  const [inputText, setInputText] = useState("");
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      text: "Hello! I'm the NSW Health Pathology Chatbot.\nCan I help you find something or answer a question?",
      timestamp: formatTime(new Date()),
      isBot: true,
    },
    {
      id: "2",
      text: "Do you still need help?",
      timestamp: formatTime(new Date()),
      isBot: true,
    },
  ]);
  const messagesEndRef = useRef<HTMLDivElement>(null);

  // Scroll to bottom when messages change
  useEffect(() => {
    if (!minimised && isOpen) {
      messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
    }
  }, [messages, minimised, isOpen]);

  if (!isOpen) return null;

  const handleSend = () => {
    const text = inputText.trim();
    if (!text) return;
    const now = new Date();
    const userMsg: Message = {
      id: Date.now().toString(),
      text,
      timestamp: formatTime(now),
      isBot: false,
    };
    setMessages((prev) => [...prev, userMsg]);
    setInputText("");

    // Simulated bot reply
    setTimeout(() => {
      const botReplies = [
        "Thanks for your message. Our team will follow up shortly.",
        "I've noted your query. Is there anything else I can help with?",
        "For urgent matters, please contact NSWHP support directly.",
        "That's a great question. Let me connect you with the right team.",
      ];
      const reply = botReplies[Math.floor(Math.random() * botReplies.length)];
      setMessages((prev) => [
        ...prev,
        {
          id: (Date.now() + 1).toString(),
          text: reply,
          timestamp: formatTime(new Date()),
          isBot: true,
        },
      ]);
    }, 900);
  };

  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") handleSend();
  };

  const panelHeight = minimised ? "auto" : 670;

  return (
    <div
      style={{
        position: "fixed",
        bottom: 0,
        right: 0,
        width: "100%",
        maxWidth: 420,
        height: minimised ? "auto" : panelHeight,
        maxHeight: minimised ? "auto" : 670,
        backgroundColor: "#ffffff",
        boxShadow: "-2px 0px 12px rgba(0,0,0,0.15)",
        zIndex: 1001,
        display: "flex",
        flexDirection: "column",
        borderRadius: "8px 0px 0px 0px",
        overflow: "hidden",
        fontFamily: "'Public Sans', sans-serif",
        transition: "height 0.25s ease",
      }}
      role="dialog"
      aria-label="NSW Health Pathology Chatbot"
      aria-modal="false"
    >
      {/* ── Header ── */}
      <div
        style={{
          backgroundColor: "#4A9BA8",
          color: "#ffffff",
          padding: "16px 20px",
          display: "flex",
          alignItems: "center",
          justifyContent: "space-between",
          flexShrink: 0,
          borderRadius: "8px 0px 0px 0px",
        }}
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div
            style={{
              width: 40,
              height: 40,
              backgroundColor: "#ffffff",
              borderRadius: "50%",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              flexShrink: 0,
            }}
          >
            <NswLogo size={24} />
          </div>
          <span style={{ fontWeight: 600, fontSize: 15 }}>
            NSW Health Pathology Chatbot
          </span>
        </div>

        <div style={{ display: "flex", gap: 4 }}>
          {/* Minimise */}
          <button
            onClick={() => setMinimised((m) => !m)}
            aria-label={minimised ? "Restore chatbot" : "Minimise chatbot"}
            style={{
              background: "transparent",
              border: "none",
              color: "#ffffff",
              cursor: "pointer",
              padding: 4,
              borderRadius: 4,
              display: "flex",
              alignItems: "center",
            }}
          >
            <svg
              width={20}
              height={20}
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth={2}
            >
              {minimised ? (
                <>
                  <polyline points="18 15 12 9 6 15" />
                </>
              ) : (
                <line x1="5" y1="12" x2="19" y2="12" />
              )}
            </svg>
          </button>
          {/* Close */}
          <button
            onClick={onClose}
            aria-label="Close chatbot"
            style={{
              background: "transparent",
              border: "none",
              color: "#ffffff",
              cursor: "pointer",
              padding: 4,
              borderRadius: 4,
              display: "flex",
              alignItems: "center",
            }}
          >
            <svg
              width={20}
              height={20}
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth={2}
            >
              <circle cx="12" cy="12" r="10" />
              <line x1="9" y1="9" x2="15" y2="15" />
              <line x1="15" y1="9" x2="9" y2="15" />
            </svg>
          </button>
        </div>
      </div>

      {/* ── Messages area ── */}
      {!minimised && (
        <div
          style={{
            flex: 1,
            overflowY: "auto",
            padding: "20px 20px 12px",
            backgroundColor: "#F5F6F8",
          }}
        >
          {/* Date label */}
          <div
            style={{
              textAlign: "center",
              color: "#6D7579",
              fontSize: 12,
              marginBottom: 20,
            }}
          >
            Today
          </div>

          {messages.map((msg) =>
            msg.isBot ? (
              /* Bot message */
              <div
                key={msg.id}
                style={{ marginBottom: 20, display: "flex", gap: 10, alignItems: "flex-start" }}
              >
                <BotAvatar />
                <div>
                  <div
                    style={{
                      backgroundColor: "#ffffff",
                      padding: "12px 16px",
                      borderRadius: "12px 12px 12px 4px",
                      maxWidth: 280,
                      boxShadow: "0px 1px 2px rgba(0,0,0,0.05)",
                    }}
                  >
                    <p
                      style={{
                        margin: 0,
                        fontSize: 14,
                        lineHeight: 1.5,
                        color: "#22272B",
                        whiteSpace: "pre-line",
                      }}
                    >
                      {msg.text}
                    </p>
                  </div>
                  <div
                    style={{
                      fontSize: 11,
                      color: "#6D7579",
                      marginTop: 4,
                      marginLeft: 4,
                    }}
                  >
                    {msg.timestamp}
                  </div>
                </div>
              </div>
            ) : (
              /* User message */
              <div
                key={msg.id}
                style={{
                  marginBottom: 20,
                  display: "flex",
                  justifyContent: "flex-end",
                }}
              >
                <div>
                  <div
                    style={{
                      backgroundColor: "#4A9BA8",
                      color: "#ffffff",
                      padding: "12px 16px",
                      borderRadius: "12px 12px 4px 12px",
                      maxWidth: 280,
                      boxShadow: "0px 1px 2px rgba(0,0,0,0.05)",
                    }}
                  >
                    <p
                      style={{
                        margin: 0,
                        fontSize: 14,
                        lineHeight: 1.5,
                        whiteSpace: "pre-line",
                      }}
                    >
                      {msg.text}
                    </p>
                  </div>
                  <div
                    style={{
                      fontSize: 11,
                      color: "#6D7579",
                      marginTop: 4,
                      textAlign: "right",
                      marginRight: 4,
                    }}
                  >
                    {msg.timestamp}
                  </div>
                </div>
              </div>
            )
          )}
          <div ref={messagesEndRef} />
        </div>
      )}

      {/* ── Input bar ── */}
      {!minimised && (
        <div
          style={{
            padding: "16px 20px",
            backgroundColor: "#ffffff",
            borderTop: "1px solid #E4E7EB",
            flexShrink: 0,
          }}
        >
          <div
            style={{
              display: "flex",
              alignItems: "center",
              gap: 8,
              backgroundColor: "#F5F6F8",
              borderRadius: 24,
              padding: "8px 16px",
            }}
          >
            {/* Emoji button */}
            <button
              aria-label="Add emoji"
              style={{
                background: "transparent",
                border: "none",
                cursor: "pointer",
                padding: 0,
                display: "flex",
                alignItems: "center",
                color: "#6D7579",
                flexShrink: 0,
              }}
            >
              <svg
                width={20}
                height={20}
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth={2}
              >
                <circle cx="12" cy="12" r="10" />
                <path d="M8 14s1.5 2 4 2 4-2 4-2" />
                <line x1="9" y1="9" x2="9.01" y2="9" />
                <line x1="15" y1="9" x2="15.01" y2="9" />
              </svg>
            </button>

            {/* Text input */}
            <input
              type="text"
              placeholder="Type a Message"
              value={inputText}
              onChange={(e) => setInputText(e.target.value)}
              onKeyDown={handleKeyDown}
              style={{
                flex: 1,
                border: "none",
                outline: "none",
                backgroundColor: "transparent",
                fontSize: 14,
                color: "#22272B",
                fontFamily: "'Public Sans', sans-serif",
              }}
              aria-label="Chat message input"
            />

            {/* Send button */}
            <button
              onClick={handleSend}
              aria-label="Send message"
              style={{
                background: "transparent",
                border: "none",
                cursor: "pointer",
                padding: 0,
                display: "flex",
                alignItems: "center",
                color: "#4A9BA8",
                flexShrink: 0,
              }}
            >
              <svg width={20} height={20} viewBox="0 0 24 24" fill="currentColor">
                <path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z" />
              </svg>
            </button>
          </div>
        </div>
      )}
    </div>
  );
}
